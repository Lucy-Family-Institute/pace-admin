  ...
  public static mapToNormedPersons(people: Array<any>) : Array<NormedPerson> {
    ...
    const normedPersons = _.map(people, (person) => {
      ...
      let np: NormedPerson = {
        ...
        sourceIds: { 
          semanticScholarIds: JSON.parse(person.semantic_scholar_ids),
          googleScholarIds: JSON.parse(person.google_scholar_ids) 
        }
      }
      return np
    })
  ...
  public static mapToCenterMembers(members: Array<any>) : NormedCenterMember[] {
    ...
    _.each(members, (member) => {
      const normedMember: NormedCenterMember = {
        ...
        sourceIds: { 
          semanticScholarIds: member.person.semantic_scholar_ids,
          googleScholarIds: member.person.google_scholar_ids  
        }
      }
