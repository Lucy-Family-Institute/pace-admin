...
import { ScopusDataSource } from './modules/scopusDataSource'
...
async function main (): Promise<void> {
  ...
  const harvestYearStr = process.env.SCOPUS_HARVEST_YEARS
  ...
  const dsConfig: DataSourceConfig = {
    baseUrl: process.env.SCOPUS_BASE_URL,
    queryUrl: process.env.SCOPUS_QUERY_URL,
    apiKey: process.env.SCOPUS_API_KEY,
    sourceName: process.env.SCOPUS_SOURCE_NAME,
    pageSize: process.env.SCOPUS_PAGE_SIZE,  // page size must be a string for the request to work,
  ...
  const ds: ScopusDataSource = new ScopusDataSource(dsConfig)
