...
function createFuzzyIndex (testKeys, funderMap) {
  ...
  const testFunderMap = _.map(funderMap, (funder) => {
    return Normalizer.normalizeObjectProperties(funder, testKeys, { skipLower: true })
 })
...
function funderMatchFuzzy (funderName, fuzzyIndex){
  ...
  const testName = Normalizer.normalizeString(funderName, { skipLower: true })
...
async function main (): Promise<void> {
  ...
  let funderMap = _.map(funders, (funder) => {
    return Normalizer.normalizeObjectProperties(funder, ['name', 'short_name'])
  })

  const subfunderMap = _.map(subfunders, (subfunder) => {
    return Normalizer.normalizeObjectProperties(subfunder, ['name', 'short_name'])
  })
  ...
  const funderVariancesMap = _.map(funderNameVariances, (funder) => {
    return Normalizer.normalizeObjectProperties(funder, ['name'])
  })

  const subfunderVariancesMap = _.map(subfunderNameVariances, (subfunder) => {
    return Normalizer.normalizeObjectProperties(subfunder, ['name'])
  })
  ...
  await pMap(subset[0], async (award) => {
    ...
    if (award['funder_name']) {
      ...
      const testFunder = Normalizer.normalizeString(award['funder_name'], { skipLower: true })
