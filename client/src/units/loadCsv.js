 ...
 */
async function loadCsv (filePath, lowerCaseColumns, columnNameMap, includeData, pageOffset, pageSize) {
  if (!columnNameMap) {
    columnNameMap = {}
  }
  if (!pageOffset) {
  ...
  if (!fs.existsSync(filePath)) {
    console.log(`Invalid path on load csv from: ${filePath}`)
    return
  }
  ...
  if (!includeData) {
    ...
    if (lowerCaseColumns || _.keys(lowerColumnNameMap).length > 0) {
      ...
      console.log(`Data is: ${JSON.stringify(curPage, null, 2)}`)
      return curPage
    }
  }
}
 ...
 */
function normalizeColumns (rows, lowerCaseColumns, columnNameMap) {
  if (!rows) {
    rows = []
  }
  if (!columnNameMap) {
...
function getTargetColumnName (originalName, lowerCaseColumns, columnNameMap) {
  if (!columnNameMap) columnNameMap = {}
